...
void transform_input(float *transformed_tile, float *input_tile, float *B, float *B_t)
{
    ...
    gemm_cpu_noblas_seq<float>(tmp, input_tile, B, 4, 4, 4);
    gemm_cpu_noblas_seq<float>(transformed_tile, B_t, tmp, 4, 4, 4);
    delete tmp;
};
...
void transform_filter(float *transformed_filter, float *filter, float *G, float *G_t)
{
    ...
    gemm_cpu_noblas_seq<float>(tmp, filter, G_t, 3, 4, 3);
    gemm_cpu_noblas_seq<float>(transformed_filter, G, tmp, 4, 4, 3);
    delete tmp;
}
...
void transform_output(float *output, float *transformed_tile, float *transformed_filter, float *A, float *A_t)
{
    ...
    gemm_cpu_noblas_seq<float>(tmp, transformed_tile, A, 4, 2, 4); // 4*4*4*2 => M = 4, N=2, K=4 => 4*2
    gemm_cpu_noblas_seq<float>(output, A_t, tmp, 2, 2, 4);         // 2*4*4*2 => M = 2, N=2, K=4 => 2*2
    delete tmp;
}
